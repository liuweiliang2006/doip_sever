...
#define PS_FREE_RTOS  //pubsub.c use freertos
...
static size_t ps_queue_space(ps_queue_t *q) {
	return uxQueueSpacesAvailable(q->queue);
}

int ps_get_space(ps_subscriber_t *su) {
	return ps_queue_space(su->q);
}
